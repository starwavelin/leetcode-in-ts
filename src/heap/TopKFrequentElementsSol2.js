 ...
 *
 * Here we can find the supported js libraries used on LeetCode:
 *   https://support.leetcode.com/hc/en-us/articles/360011833974-What-are-the-environments-for-the-programming-languages
 *
 * Solution Variation - using existing PriorityQueue data structure available on LeetCode,
 *  which is actually from the Version 5 of https://github.com/datastructures-js/priority-queue/blob/v5/README.md
 */
var topKFrequentPriorityQueue = function (nums, k) {
  const map = new Map();
  for (const n of nums) {
    map.set(n, (map.get(n) || 0) + 1);
  }

  // build the max heap using the given library
  const maxHeap = new PriorityQueue({ compare: (a, b) => b.freq - a.freq });
  for (let [key, freq] of map) {
    maxHeap.enqueue({ key, freq });
  }

  const res = [];
  for (let i = 0; i < k; i++) {
    res.push(maxHeap.dequeue().key);
  }
...
/**
 * Solution 2: The hand-build max heap apporach
 */
...
const arr2 = [1, 1, 1, 2, 2, 3, 1, 1, 1, 1, 3, 3, 3, 3, 3, 3, 5, 5, 5, 5, 5, 5, 5, 5, 5],
  k2 = 3;
console.log(topKFrequent(arr2, k2)); // expect [5, 1, 3] or similar  5: 9times, 3: 7times, 1: 7times

const arr3 = [4, 1, -1, 2, -1, 2, 3],
  k3 = 2;
